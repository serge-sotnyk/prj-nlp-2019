...
def calc_genome_fitness(genome: List[int], tokens: List[str], classifier: Pipeline) -> float:
    fitness = 0
    for i, head_pos in enumerate(genome):
        child_pos = i + 1
        f = relation_to_features(tokens, child_pos, head_pos)
        prediction = classifier.predict_proba([f])
        fitness += prediction[0][1] - prediction[0][0]
    fitness -= _calc_penalties(genome)
    return fitness


def generate_new_generation(sorted_genomes, mutation_prob)->List[List[int]]:
    result = []
...
def restore_genome(tokens: List[str], classifier: Pipeline, verbose: bool=True) -> List[int]:
    ...
    for epoch in range(max_epoch):
        fitnesses = [calc_genome_fitness(genome, tokens, classifier) for genome in pool]
        ...
        pool = generate_new_generation(sorted_genomes, mutation_prob)
    return pool[0]
